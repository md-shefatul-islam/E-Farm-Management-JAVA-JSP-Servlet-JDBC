...
public class SeedDb {
    ...
    private static final String CREATE_CROP_SQL = "INSERT INTO farmtech.seed (name, description, quantity, price, date, funame) VALUES (?, ?, ?, ?, ?, ?);";

    private static final String SELECT_SEED_BY_ID = "select * from farmtech.seed where id = ?;";
    private static final String SELECT_SEED_BY_NAME = "select * from farmtech.seed where suname = ?;";
    private static final String SELECT_ALL_SEED = "select * from farmtech.seed;";
    ...
    private static final String UPDATE_CROP_QUANTITY = "update farmtech.seed set quantity = ? where id = ?;";
    ...
    public List<Seed> allseed() {
        ...
        try (Connection connection = dbc.getConnection();
             ...
             PreparedStatement ps = connection.prepareStatement(SELECT_ALL_SEED)) {
//            ps.setString(1, uname);
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    seed.add(new Seed(id,name,description,quantity,price,date,suname)) ;
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public Seed selectSeed(int cid) {
        Seed seed = null;

        try (Connection connection = dbc.getConnection();

             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_SEED_BY_ID)) {
            ...
            while (rs.next()) {
                ...
                seed = new Seed(id,name,description,quantity,price,date,suname) ;
    ...
    public Seed selectseed(String names) {
        Seed seed = null;

        try (Connection connection = dbc.getConnection();

             PreparedStatement preparedStatement = connection.prepareStatement(SELECT_SEED_BY_NAME)) {
    ...
    public boolean updatequantity(Seed seed) {
        //boolean rowUpdated = false;
        try (Connection connection = dbc.getConnection();
             PreparedStatement ps = connection.prepareStatement(UPDATE_CROP_QUANTITY)) {

            ps.setString(1, seed.getQuantity());
            ps.setInt(2, seed.getId());
