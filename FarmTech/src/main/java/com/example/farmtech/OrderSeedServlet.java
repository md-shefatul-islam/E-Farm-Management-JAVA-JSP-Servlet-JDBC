...
@WebServlet(name = "orderseed", value = "/orderseed")
public class OrderSeedServlet extends HttpServlet {
...
//        Seed seed = sdb.selectCrop(id);
        ...
        if(null == session.getAttribute("orderid")){
            ...
            if(ids.equals(iddd)){
                session.setAttribute("orderid", id);
//                session.setAttribute("orderquantity", "i am no where");
            }else{
//                session.setAttribute("orderquantity", "i am here   "+ids+"   "+session.getAttribute("orderid"));
        ...
        RequestDispatcher dispatcher = request.getRequestDispatcher("order/seedorder.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            Seed seed = sdb.selectSeed(id);

            int quantity = Integer.parseInt(request.getParameter("quantity"));
            String quantitiess = request.getParameter("quantity");
//            out.println("allah is with me"+id+quantity+seed.getName());
            int oldquantity = Integer.parseInt(seed.getQuantity());
            if( oldquantity >= quantity){
                int price = Integer.parseInt(seed.getPrice());
...
//                    out.println("allah is sorbo mohan");
                ...
                session.setAttribute("ordersuname", seed.getSuname());

                RequestDispatcher dispatcher = request.getRequestDispatcher("order/confirmseedorder.jsp");
                request.setAttribute("order", seed);
                dispatcher.forward(request, response);
            }
            else{
                RequestDispatcher dispatcher = request.getRequestDispatcher("order/seedorder.jsp");
