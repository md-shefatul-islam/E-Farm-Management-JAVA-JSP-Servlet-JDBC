                            ...
                            <div class="card shadow mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <c:forEach var="allseed" items='<%= request.getAttribute("allseed")%>'>
                                            <tr>
<%--                                                <c:set var="username" scope="session" value='<%= session.getAttribute("username")%>' />--%>
<%--                                                <c:if test="${allseed.funame != username}">--%>

                                                <td><c:out value="${allseed.id}"/></td>
                                                <td><c:out value="${allseed.name}"/></td>
                                                <td><c:out value="${allseed.description}"/></td>
                                                <td><c:out value="${allseed.quantity}"/></td>
                                                <td><c:out value="${allseed.price}"/></td>
                                                <td><c:out value="${allseed.date}"/></td>
                                                <td>
                                                    <c:set var="msg" scope="session" value='<%= session.getAttribute("orderid")%>'>
                                                    </c:set>
                                                    <c:choose>
                                                        <c:when test="${msg.equals(allseed.id)}">
                                                            <c:choose>
                                                                <c:when test="${cookie.name.value == 'close'}">


                                                                    <a type="button" class="btn btn-success" style="color: aliceblue" href="<%=request.getContextPath()%>/orderseed?id=${allseed.id}">Edit</a>
                                                                    <a type="button" class="btn btn-success" style="color: aliceblue" href="<%=request.getContextPath()%>/deleteseedorder?id=${allseed.id}">Delete</a>
                                                             </c:when>
                                                                <c:otherwise>
                                                                    <a type="button" class="btn btn-success" style="color: aliceblue" href="<%=request.getContextPath()%>/orderseed?id=${allseed.id}">Order</a>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </c:when>
